...
(d.message||d.code||"")),this.tracker.trackAction("controller-events","error",c.type,-1)):(this.notifySuccess("Your site is published. "),this.tracker.trackAction("controller-events","success",c.type,1))},this),goog.bind(function(d){console.error("Error: I did not manage to publish the file. (2)",d);this.notifyError("I did not manage to publish the file. You may want to check the publication settings and your internet connection. <br /><br />Error message: "+d);this.tracker.trackAction("controller-events",
"error",c.type,-1)},this)):(this.publishSettings.openDialog(),this.workspace.invalidate());break;case "file.save":this.file.getBlob()?this.file.save(goog.bind(function(){this.notifySuccess("Your file is saved.");this.tracker.trackAction("controller-events","success",c.type,1)},this),goog.bind(function(d){this.notifyError("Error: I did not manage to save the file. <br /><br />"+(d.message||""));this.tracker.trackAction("controller-events","error",c.type,-1)},this)):this.file.saveAs(goog.bind(function(){this.notifySuccess("Your file is saved.");
this.tracker.trackAction("controller-events","success",c.type,1)},this),goog.bind(function(d){this.notifyError("Error: I did not manage to save the file. <br /><br />"+(d.message||""));this.tracker.trackAction("controller-events","error",c.type,-1)},this));break;case "file.open":this.file.open(goog.bind(function(){this.notifySuccess(this.file.getTitle()+" opened.");this.selection.setComponent(this.file.getStageComponent());this.tracker.trackAction("controller-events","success",c.type,1)},this),goog.bind(function(d){this.notifyError("Error: I did not manage to open this file. <br /><br />"+
(d.message||""));this.tracker.trackAction("controller-events","error",c.type,-1)},this));break;case "file.close":this.file.close();this.selection.setComponent(null);break;case "view.file":this.file.view();break;case "tools.advanced.activate":goog.dom.classes.has(document.body,"advanced-mode-on")?(goog.dom.classes.remove(document.body,"advanced-mode-on"),goog.dom.classes.add(document.body,"advanced-mode-off")):(goog.dom.classes.add(document.body,"advanced-mode-on"),goog.dom.classes.remove(document.body,
"advanced-mode-off"));break;case "tools.debug.activate":this.logger.getLevel()!==silex.Logger.ALL?this.logger.setLevel(silex.Logger.ALL):this.logger.setLevel(silex.Logger.OFF);break;case "view.open.fileExplorer":this.fileExplorer.openDialog();this.workspace.invalidate();break;case "view.open.textEditor":this.editComponent();break;case "insert.page":this.createPage(function(){this.tracker.trackAction("controller-events","success",c.type,1)},function(){this.tracker.trackAction("controller-events","cancel",
c.type,0)});break;case "insert.text":var d=this.file.getStageComponent().addText();this.selection.getPage().addComponent(d);this.selection.setComponent(d);break;case "insert.html":d=this.file.getStageComponent().addHtml();this.selection.getPage().addComponent(d);this.selection.setComponent(d);break;case "insert.image":this.fileExplorer.openDialog(goog.bind(function(d){d=this.file.getStageComponent().addImage(d.url);this.selection.getPage().addComponent(d);this.selection.setComponent(d);this.tracker.trackAction("controller-events",
"success",c.type,1)},this),["image/*","text/plain"],["jpg","gif","png"],goog.bind(function(d){this.notifyError("Error: I did not manage to load the image. <br /><br />"+(d.message||""));this.tracker.trackAction("controller-events","error",c.type,-1)},this));this.workspace.invalidate();break;case "insert.container":d=this.file.getStageComponent().addContainer();this.selection.getPage().addComponent(d);this.selection.setComponent(d);break;case "edit.delete.selection":this.file.getStageComponent().remove(this.selection.getComponent());
this.selection.setComponent(this.file.getStageComponent());break;case "edit.delete.page":this.removePage(this.selection.getPage());break;case "edit.rename.page":this.renamePage(this.selection.getPage());break;case "help.about":window.open("http://www.silexlabs.org/silex/");break;case "help.aboutSilexLabs":window.open("http://www.silexlabs.org/silexlabs/");break;case "help.forums":window.open("http://graphicdesign.stackexchange.com/questions/tagged/silex");break;case "help.newsLetter":window.open("http://eepurl.com/F48q5");
break;case "help.googlPlus":window.open("https://plus.google.com/communities/107373636457908189681");break;case "help.twitter":window.open("http://twitter.com/silexlabs");break;case "help.facebook":window.open("http://www.facebook.com/silexlabs");break;case "help.forkMe":window.open("https://github.com/silexlabs/Silex")}};
...
c);goog.events.listen(this.textField,goog.editor.Field.EventType.DELAYEDCHANGE,function(){this.contentChanged()},!1,this);try{this.textField.makeEditable()}catch(f){console.error("error catched",f,goog.editor.BrowserFeature.HAS_STYLE_WITH_CSS)}goog.events.listen(goog.dom.getElementByClass("close-btn",this.element),goog.events.EventType.CLICK,function(){this.closeEditor()},!1,this)};
