...
goog.ui.KeyboardShortcutHandler.EventType.SHORTCUT_TRIGGERED,goog.bind(function(c){console.log("key",arguments,c.target.tagName,goog.events.KeyCodes.ENTER);c.preventDefault();this.onMenuEvent(c.identifier)},this));h=new goog.events.KeyHandler(document);goog.events.listen(h,"key",goog.bind(function(c){console.log("key",arguments,c.target.tagName,goog.events.KeyCodes.ENTER);c.keyCode===goog.events.KeyCodes.ENTER&&("INPUT"===c.target.tagName||c.target.tagName===d.textInputs_[c.target.type]?console.log("stop",
c.target.tagName,c.target.type):(c.preventDefault(),this.onMenuEvent("view.open.textEditor")))},this));this.menu.render(c);goog.events.listen(this.menu,goog.ui.Component.EventType.ACTION,function(c){this.onClick(c)},!1,this);goog.events.listen(goog.dom.getElementByClass("website-name"),goog.events.EventType.CLICK,function(c){this.onClick(c)},!1,this)};
silex.view.Menu.prototype.onClick=function(c){if(this.onStatus&&c&&c.target)if(goog.dom.classes.has(c.target,"website-name"))this.onMenuEvent("title.changed");else this.onMenuEvent(c.target.getId())};silex.view.Menu.prototype.onMenuEvent=function(c){if(this.onStatus)this.onStatus({type:c})};silex.view.Menu.prototype.setWebsiteName=function(c){goog.dom.getElementByClass("website-name").innerHTML=c};silex.view.Menu.prototype.getWebsiteName=function(){return goog.dom.getElementByClass("website-name").innerHTML};silex.view.propertiesTool={};silex.view.propertiesTool.PropertyPane=function(c,d,e,f,h){this.element=c;this.propertyChanged=d;this.editText=f;this.editHTML=e;this.selectImage=h;this.buildUi()};
...
!1,this);goog.events.listen(this.element,goog.events.EventType.DBLCLICK,function(c){if(this.onStatus)this.onStatus({type:"edit"})},!1,this);handleMouseWheel=goog.bind(function(c){0>c.deltaX&&0>=this.element.scrollLeft&&(console.log("prevent mouse wheel",c,this.element.scrollLeft),c.preventDefault())},this);var e=new goog.events.MouseWheelHandler(this.element);goog.events.listen(e,goog.events.MouseWheelHandler.EventType.MOUSEWHEEL,handleMouseWheel);goog.events.listen(window,"unload",function(c){goog.events.unlisten(e,
goog.events.MouseWheelHandler.EventType.MOUSEWHEEL,handleMouseWheel)})};silex.view.Stage.prototype.getPagesNamesFromDom=function(){var c=[];$('a[data-silex-type="page"]',this.bodyElement).each(function(){c.push(this.getAttribute("id"))});return c};
...
c);this.setEditable(!0,c);c=new silex.model.Component(c);d={};d.left=100+this.element.scrollLeft+"px";d.top=100+this.element.scrollTop+"px";d.height="100px";d.width="100px";c.setBoundingBox(d);c.setStyle({backgroundColor:"rgba(255, 255, 255, 1)"},silex.model.Component.CONTEXT_NORMAL);return c};
...
goog.style.setStyle(e,"width","100%");goog.style.setStyle(e,"height","100%");var f=this.getBackgroundElement();goog.dom.appendChild(f,d);goog.dom.appendChild(d,e);var h=new silex.model.Component(d),k={};k.left=100+this.element.scrollLeft+"px";k.top=100+this.element.scrollTop+"px";h.setBoundingBox(k);e.setAttribute("src",c);e.onload=function(c){k.width=e.offsetWidth+"px";k.height=e.offsetHeight+"px";h.setBoundingBox(k);h.setEditable(!0,d)};return h};
...
f,d));this.publishSettings.element&&(d=goog.style.getSize(this.publishSettings.element),f=(c.width-d.width)/2,d=(c.height-d.height)/2,goog.style.setPosition(this.publishSettings.element,f,d));goog.dom.classes.has(document.body,"loading-pending")&&goog.dom.classes.remove(document.body,"loading-pending")};silex.view.PropertiesTool=function(c,d){this.element=c;this.context=silex.model.Component.CONTEXT_NORMAL;silex.Helper.loadTemplateFile("templates/propertiestool.html",c,function(){this.buildTabs();this.buildPanes();d&&d()},this)};silex.view.PropertiesTool.TAB_TITLE_NORMAL="Normal";silex.view.PropertiesTool.TAB_TITLE_HOVER="Hover";silex.view.PropertiesTool.TAB_TITLE_PRESSED="Pressed";
