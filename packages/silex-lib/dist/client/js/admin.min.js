...
silex.model.Element.prototype.getInnerHtml=function(c){silex.utils.JQueryEditable.setEditable(c,!1);var d=this.getContentNode(c).innerHTML;silex.utils.JQueryEditable.setEditable(c,!0);return d};silex.model.Element.prototype.setInnerHtml=function(c,d){silex.utils.JQueryEditable.setEditable(c,!1);this.getContentNode(c).innerHTML=d;silex.utils.JQueryEditable.setEditable(c,!0)};
silex.model.Element.prototype.getContentNode=function(c){var d=goog.dom.getElementsByClass(silex.model.Element.ELEMENT_CONTENT_CLASS_NAME);return d&&1===d.length?d[0]:c};silex.model.Element.prototype.setContentNode=function(c,d){var e=this.getContentNode(c);e===c?(silex.utils.JQueryEditable.setEditable(c,!1),c.innerhtml=d.innerhtml,silex.utils.JQueryEditable.setEditable(c,!0)):goog.dom.replaceNode(d,e)};
...
silex.view.Menu.prototype.getWebsiteName=function(){return goog.dom.getElementByClass(headElement,"website-name").innerHTML};silex.utils={};silex.utils.JQueryPageable=function(){throw"this is a static class and it canot be instanciated";};silex.utils.JQueryPageable.LINK_ATTR="data-silex-href";silex.utils.JQueryPageable.PAGEABLE_ROOT_CLASS="pageable-root-class";silex.utils.JQueryPageable.PAGE_CLASS="silex-page";silex.utils.JQueryPageable.getParentPage=function(c){for(c=c.parentNode;c&&!goog.dom.classes.has(c,silex.utils.JQueryPageable.PAGE_CLASS);)c=c.parentNode;return c};
silex.utils.JQueryPageable.getPageable=function(c){return c&&$(c).hasClass(silex.utils.JQueryPageable.PAGEABLE_ROOT_CLASS)};silex.utils.JQueryPageable.setPageable=function(c,d){if(d){$(c).addClass(silex.utils.JQueryPageable.PAGEABLE_ROOT_CLASS);var e=silex.utils.JQueryPageable.getPages(c);$(c).pageable({currentPage:e[0],useDeeplink:!1})}else $(c).pageable("destroy"),$(c).removeClass(silex.utils.JQueryPageable.PAGEABLE_ROOT_CLASS)};
silex.utils.JQueryPageable.getPages=function(c){if(!silex.utils.JQueryPageable.getPageable(c))throw Error("Operation failed, root pageable element is required.");var d=[];$('a[data-silex-type="page"]',c).each(function(){d.push(this.getAttribute("id"))});return d};silex.utils.JQueryPageable.getCurrentPageName=function(c){if(!silex.utils.JQueryPageable.getPageable(c))throw Error("Operation failed, root pageable element is required.");return $(c).pageable("option","currentPage")};
silex.utils.JQueryPageable.setCurrentPage=function(c,d){if(!silex.utils.JQueryPageable.getPageable(c))throw Error("Operation failed, root pageable element is required.");$(c).pageable({currentPage:d})};silex.utils.JQueryPageable.getDisplayName=function(c,d){if(!silex.utils.JQueryPageable.getPageable(c))throw Error("Operation failed, root pageable element is required.");var e="";$('a[data-silex-type="page"]',c).each(function(){this.getAttribute("id")===d&&(e=this.innerHTML)});return e};
silex.utils.JQueryPageable.removePage=function(c,d){if(!silex.utils.JQueryPageable.getPageable(c))throw Error("Operation failed, root pageable element is required.");$('a[data-silex-type="page"]',c).each(function(){this.getAttribute("id")===d&&$(this).remove()});$('*[data-silex-href="#!'+d+'"]').each(function(){this.removeAttribute("data-silex-href")});$("."+d).each(function(){$(this).removeClass(d);0>=silex.utils.JQueryPageable.getPagesForElement(c,this).length&&$(this).removeClass(silex.utils.JQueryPageable.PAGE_CLASS)})};
silex.utils.JQueryPageable.createPage=function(c,d,e){if(!silex.utils.JQueryPageable.getPageable(c))throw Error("Operation failed, root pageable element is required.");var f=goog.dom.createElement("a");f.setAttribute("id",d);f.setAttribute("data-silex-type","page");f.innerHTML=e;goog.dom.appendChild(c,f)};
silex.utils.JQueryPageable.renamePage=function(c,d,e,f){if(!silex.utils.JQueryPageable.getPageable(c))throw Error("Operation failed, root pageable element is required.");$('a[data-silex-type="page"]',c).each(function(){this.getAttribute("id")===d&&(this.setAttribute("id",e),this.innerHTML=f)});$('*[data-silex-href="#!'+d+'"]').each(function(){this.setAttribute("data-silex-href","#!"+e)});$("."+d).each(function(){$(this).removeClass(d);$(this).addClass(e)})};
silex.utils.JQueryPageable.setLink=function(c,d){d?c.setAttribute(silex.utils.JQueryPageable.LINK_ATTR,d):c.removeAttribute(silex.utils.JQueryPageable.LINK_ATTR)};silex.utils.JQueryPageable.getLink=function(c){return c.getAttribute("data-silex-href")};silex.utils.JQueryPageable.addToPage=function(c,d,e){goog.dom.classes.add(d,e);goog.dom.classes.add(d,silex.utils.JQueryPageable.PAGE_CLASS)};
silex.utils.JQueryPageable.removeFromPage=function(c,d,e){goog.dom.classes.remove(d,e);0<!silex.utils.JQueryPageable.getPagesForElement(c,d).length&&goog.dom.classes.remove(d,silex.utils.JQueryPageable.PAGE_CLASS)};silex.utils.JQueryPageable.getPagesForElement=function(c,d){if(!silex.utils.JQueryPageable.getPageable(c))throw Error("Operation failed, root pageable element is required.");var e=[],f=silex.utils.JQueryPageable.getPages(c);for(idx in f){var h=f[idx];goog.dom.classes.has(d,h)&&e.push(h)}return e};
silex.utils.JQueryPageable.isInPage=function(c,d,e){e||(e=silex.utils.JQueryPageable.getCurrentPageName(c));return goog.dom.classes.has(d,e)};silex.utils.Notification=function(){throw"this is a static class and it canot be instanciated";};silex.utils.Notification.ERROR_ICON="assets/notifications/error.png";silex.utils.Notification.SUCCESS_ICON="assets/notifications/success.png";silex.utils.Notification.INFO_ICON="assets/notifications/info.png";silex.utils.Notification.useNative=function(){return window.webkitNotifications&&0===window.webkitNotifications.checkPermission()};
...
"text/plain"],goog.bind(function(c){silex.utils.Notification.notifyError("Error: I did not manage to load the image. <br /><br />"+(c.message||""))},this)),this.view.workspace.invalidate()}};silex.controller.ControllerBase.prototype.openPage=function(c){silex.utils.JQueryPageable.setCurrentPage(this.model.body.bodyElement,c);this.view.pageTool.redraw()};
silex.controller.ControllerBase.prototype.renamePage=function(c){c||(c=silex.utils.JQueryPageable.getCurrentPageName(this.model.body.bodyElement));this.getUserInputPageName(silex.utils.JQueryPageable.getDisplayName(this.model.body.bodyElement,c),goog.bind(function(d,e){e&&(silex.utils.JQueryPageable.renamePage(this.model.body.bodyElement,c,d,e),this.openPage(d))},this))};
silex.controller.ControllerBase.prototype.removePage=function(c){c||(c=silex.utils.JQueryPageable.getCurrentPage(this.model.body.bodyElement));silex.utils.Notification.confirm('I am about to delete the page "'+silex.utils.JQueryPageable.getDisplayName(this.model.body.bodyElement,c)+'", are you sure?',goog.bind(function(d){d&&(silex.utils.JQueryPageable.removePage(this.model.body.bodyElement,c),this.view.pageTool.redraw(),this.view.propertyTool.redraw())},this))};
silex.controller.ControllerBase.prototype.getUserInputPageName=function(c,d){silex.utils.Notification.prompt("Enter a name for your page!",goog.bind(function(c,f){if(c&&f&&0<f.length){var h=f;f=f.replace(/\ /g,"-").replace(/\./g,"-").replace(/'/g,"-").replace(/"/g,"-").toLowerCase();var k=silex.utils.JQueryPageable.getPages(this.model.body.bodyElement),l=!1;goog.array.forEach(k,function(c){c===f&&(l=!0)});l?this.openPage(f):d(f,h)}d(null)},this),c)};
silex.controller.ControllerBase.prototype.checkElementVisibility=function(c){if(null!==silex.utils.JQueryPageable.getParentPage(c)){var d=silex.utils.JQueryPageable.getPagesForElement(this.model.body.bodyElement,c);for(idx in d)silex.utils.JQueryPageable.removeFromPage(this.model.body.bodyElement,c,d[idx]);this.view.propertyTool.redraw()}};
silex.controller.ControllerBase.prototype.createPage=function(c,d){this.getUserInputPageName("Your new page name",goog.bind(function(e,f){e?(silex.utils.JQueryPageable.createPage(this.model.body.bodyElement,e,f),this.openPage(e),this.tracker.trackAction("controller-events","success",event.type,0),c&&c()):(this.tracker.trackAction("controller-events","cancel",event.type,0),d&&d())},this))};
...
silex.controller.ControllerBase.prototype.fileOperationSuccess=function(c,d){var e=silex.utils.JQueryPageable.getPages(this.model.body.bodyElement);silex.utils.JQueryPageable.setCurrentPage(this.model.body.bodyElement,e[0]);d&&(this.view.pageTool.redraw(),this.view.propertyTool.redraw(),this.view.menu.redraw(),this.refreshFonts());c&&silex.utils.Notification.notifySuccess(c)};
...
function(c){0>c.deltaX&&0>=this.element.scrollLeft&&c.preventDefault()},!1,this)};silex.view.Stage.prototype.getSelection=function(){return goog.dom.getElementsByClass(silex.model.Element.SELECTED_CLASS_NAME,this.bodyElement)};silex.view.Stage.prototype.getNeededFonts=function(){var c=[];this.bodyElement.innerHTML.replace(/<font[^"]*face="?([^"]*)"/g,function(d,e,f){c[e]=!0;return d});return c};silex.controller.SettingsDialogController=function(c,d){silex.controller.ControllerBase.call(this,c,d);this.view.settingsDialog.onStatus=goog.bind(this.settingsDialogCallback,this)};goog.inherits(silex.controller.SettingsDialogController,silex.controller.ControllerBase);
...
(d.message||""));this.tracker.trackAction("controller-events","error",c,-1)},this));this.view.workspace.invalidate();break;case "selectImage":this.editElement();break;case "styleChanged":var f=this.view.stage.getSelection()[0];f&&d?(this.model.element.setStyle(f,d,e),this.view.propertyTool.redraw()):console.error("can not set style ",d," on element ",f);break;case "propertyChanged":console.error("not implemented");break;case "addToPage":silex.utils.JQueryPageable.addToPage(this.model.body.bodyElement,
this.view.stage.getSelection()[0],d);break;case "removeFromPage":silex.utils.JQueryPageable.removeFromPage(this.model.body.bodyElement,this.view.stage.getSelection()[0],d);break;case "addLink":silex.utils.JQueryPageable.setLink(this.view.stage.getSelection()[0],d);break;case "removeLink":silex.utils.JQueryPageable.setLink(this.view.stage.getSelection()[0])}};silex.controller.MenuController=function(c,d){silex.controller.ControllerBase.call(this,c,d);d.menu.onStatus=goog.bind(this.menuCallback,this)};goog.inherits(silex.controller.MenuController,silex.controller.ControllerBase);
...
silex.view.pane.PagePane.prototype.redraw=function(){silex.view.pane.PagePane.superClass_.redraw.call(this);this.setPages(silex.utils.JQueryPageable.getPages(this.bodyElement));var c=this.getSelection()[0];if(c){goog.array.forEach(this.pageCheckboxes,function(d){d.checkbox.setEnabled(!0);d.checkbox.setChecked(silex.utils.JQueryPageable.isInPage(this.bodyElement,c,d.pageName))},this);var d=silex.utils.JQueryPageable.getLink(c);console.log(d);d&&""!==d?0===d.indexOf("#!")?this.linkDropdown.value=d:
(this.linkInputTextField.setValue(d),this.linkDropdown.value="custom"):(this.linkDropdown.value="none",this.linkInputTextField.setValue(""));d=goog.dom.getElementByClass("link-input-text",this.element);"custom"===this.linkDropdown.value?goog.style.setStyle(d,"display","inherit"):goog.style.setStyle(d,"display","none");this.isRedraw=!1}};silex.view.pane.PagePane.prototype.checkPage=function(c,d){if(d.isChecked())this.onStatus("addToPage",c);else this.onStatus("removeFromPage",c)};silex.utils.Style={};silex.utils.Style.styleToString=function(c){var d="";goog.object.forEach(c,function(c,f,h){c&&(d+=goog.string.toSelectorCase(f)+": "+c+"; ")});return d};silex.utils.Style.stringToStyle=function(c){return goog.style.parseStyleAttribute(c)};
...
silex.view.PageTool.prototype.redraw=function(){var c=silex.utils.JQueryPageable.getPages(this.bodyElement),d=silex.utils.JQueryPageable.getCurrentPageName(this.bodyElement),e=0,c=c.map(goog.bind(function(c){var f={name:c,displayName:silex.utils.JQueryPageable.getDisplayName(this.bodyElement,c),linkName:"#!"+c,idx:e++};f.className=d===c?"ui-selected":"";return f},this)),f=goog.dom.getElementByClass("page-tool-container",this.element),h=goog.dom.getElementByClass("page-tool-template",this.element).innerHTML;
f.innerHTML=silex.utils.Dom.renderList(h,c)};silex.view.PageTool.prototype.removePageAtIndex=function(c){var d=silex.utils.JQueryPageable.getPages(this.bodyElement);if(this.onStatus)this.onStatus("delete",d[c]);this.redraw()};silex.view.PageTool.prototype.renamePageAtIndex=function(c){var d=silex.utils.JQueryPageable.getPages(this.bodyElement);if(this.onStatus)this.onStatus("rename",d[c]);this.redraw()};
...
silex.view.TextEditor.prototype.openEditor=function(){silex.utils.JQueryEditable.getContainer();var c=getElementByClass(silex.model.Element.SELECTED_CLASS_NAME);this.textField.setHtml(!1,c);this.textField.focusAndPlaceCursorAtStart();c=goog.dom.getElementByClass("dialogs-background");goog.style.setStyle(c,"display","inherit");goog.style.setStyle(this.element,"display","inherit")};
...
silex.model.File.prototype.setHtml=function(c){var d,e=c.toLowerCase(),f=e.indexOf("<head>");-1===f&&(f=e.indexOf("<head "));var h=e.indexOf("</head>"),k=e.indexOf("<body>");-1===k&&(k=e.indexOf("<body "));var l=e.indexOf("</body>");-1<f&&-1<h&&(f=e.indexOf(">",f),d=c.substring(f+1,h));-1<k&&-1<l&&(f=e.indexOf(">",k),c.substring(f+1,l));c=c.substring(k,l+7);this.bodyElement.innerHTML=c;this.headElement.innerHTML=d;silex.utils.JQueryEditable.setEditable(this.bodyElement,!0,!0)};
...
silex.model.Body.prototype.filterBgImage=function(c,d,e,f,h){c=silex.utils.Url.getAbsolutePath(h,c);e=silex.utils.Url.getRelativePath(c,silex.utils.Url.getBaseUrl());silex.utils.Url.isAbsoluteUrl(e)||(e=e.replace("../","/"));f="assets/"+c.substr(c.lastIndexOf("/")+1);h="url('../"+f+"')";d.push({url:c,destPath:f,srcPath:e});return h};silex.App=function(){silex.service.Tracker.getInstance().trackAction("app-events","start",null,2);"silex"===window.location.href.slice(-5)&&(window.location.href+="/");window.location.hash="";if(!silex.Config.debug.debugMode||silex.Config.debug.preventQuit)window.onbeforeunload=function(){return"Are you sure you want to leave me?"};var c=goog.dom.getElementByClass("silex-stage-body"),d=goog.dom.createElement("div"),e=goog.dom.getElementByClass("silex-menu"),f=new silex.view.Menu(e,c,d),h=goog.dom.getElementByClass("silex-stage"),
k=new silex.view.Stage(h,c,d),l=goog.dom.getElementByClass("silex-page-tool"),m=new silex.view.PageTool(l,c,d),n=goog.dom.getElementByClass("silex-htmleditor"),p=new silex.view.HTMLEditor(n,c,d),q=goog.dom.getElementByClass("silex-texteditor"),s=new silex.view.TextEditor(q,c,d),t=goog.dom.getElementByClass("silex-fileexplorer"),u=new silex.view.FileExplorer(t,c,d),v=goog.dom.getElementByClass("silex-settings-dialog"),x=new silex.view.SettingsDialog(v,c,d),w=goog.dom.getElementByClass("silex-property-tool"),
y=new silex.view.PropertyTool(w,c,d),z=goog.dom.getElementByClass("silex-workspace"),e=new silex.view.Workspace(z,e,h,l,w,n,q,t,v),c=new silex.types.Model(new silex.model.File(c,d),new silex.model.Head(c,d),new silex.model.Body(c,d),new silex.model.Element(c,d)),f=new silex.types.View(e,f,k,m,y,p,s,u,x),k=new silex.types.Controller(new silex.controller.MenuController(c,f),new silex.controller.StageController(c,f),new silex.controller.PageToolController(c,f),new silex.controller.PropertyToolController(c,
f),new silex.controller.SettingsDialogController(c,f),new silex.controller.HtmlEditorController(c,f),new silex.controller.TextEditorController(c,f));k.menuController.menuCallback("file.new");silex.Config.debug.debugMode&&silex.Config.debug.doAfterReady&&silex.Config.debug.doAfterReady(c,f,k)};goog.exportSymbol("silex.App",silex.App);
