...
silex.view.Menu.prototype.buildMenu=function(c){this.menu=goog.ui.menuBar.create();var d=new goog.ui.KeyboardShortcutHandler(document);goog.events.listen(d,goog.ui.KeyboardShortcutHandler.EventType.SHORTCUT_TRIGGERED,goog.bind(function(c){this.onMenuEvent(c.identifier)},this));for(i in silex.model.Config.menu.names){var e=new goog.ui.Menu;goog.array.forEach(silex.model.Config.menu.options[i],function(c){var f;if(c){var h=c.id;f=new goog.ui.MenuItem(c.label);f.setId(h);f.addClassName(c.className);
c.checkable&&f.setCheckable(!0);c.mnemonic&&f.setMnemonic(c.mnemonic);if(c.shortcut)for(var n in c.shortcut)d.registerShortcut(c.id,c.shortcut[n])}else f=new goog.ui.MenuSeparator;e.addItem(f);c&&c.tooltip&&(h=goog.dom.createElement("span"),h.innerHTML=c.tooltip,h.className="goog-menuitem-accel",f.getElement().appendChild(h))},this);var f=silex.model.Config.menu.names[i],h=new goog.ui.MenuButton(f.label,e);h.addClassName(f.className);h.setDispatchTransitionEvents(goog.ui.Component.State.ALL,!0);this.menu.addChild(h,
!0)}this.menu.render(c);goog.events.listen(this.menu,goog.ui.Component.EventType.ACTION,function(c){this.onClick(c)},!1,this);goog.events.listen(goog.dom.getElementByClass("website-name"),goog.events.EventType.CLICK,function(c){this.onClick(c)},!1,this)};silex.view.Menu.prototype.onClick=function(c){if(this.onStatus&&c&&c.target)if(goog.dom.classes.has(c.target,"website-name"))this.onMenuEvent("title.changed");else this.onMenuEvent(c.target.getId())};silex.view.Menu.prototype.onMenuEvent=function(c){if(this.onStatus)this.onStatus({type:c})};
