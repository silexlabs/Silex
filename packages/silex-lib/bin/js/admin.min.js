...
"success",c.type,1)},this),["image/*","text/plain"],goog.bind(function(d){this.notifyError("Error: I did not manage to load the image. <br /><br />"+(d.message||""));this.tracker.trackAction("controller-events","error",c.type,-1)},this));this.workspace.invalidate();break;case "insert.container":d=this.file.getStageComponent().addContainer();this.selection.getPage().addComponent(d);this.selection.setComponent(d);break;case "edit.delete.selection":this.file.getStageComponent().remove(this.selection.getComponent());
...
silex.Controller.prototype.publishSettingsCallback=function(c){switch(c.type){case "browsePublishPath":this.fileExplorer.openDialog(goog.bind(function(d){d=d.url.substring(d.url.indexOf("/api/v1.0/"),d.url.lastIndexOf("/"));d=d.replace("/exec/get","/exec/put");this.file.setPublicationPath(d);this.tracker.trackAction("controller-events","success",c.type,1)},this),["image/*","text/plain"],goog.bind(function(d){this.notifyError("Error: I could not select the publish path. <br /><br />"+(d.message||""));
this.tracker.trackAction("controller-events","error",c.type,-1)},this));break;case "change":this.file.setPublicationPath(c.data)}};
...
silex.view.FileExplorer.prototype.init=function(){this.filePicker=silex.service.CloudStorage.getInstance().filePicker};silex.view.FileExplorer.prototype.openDialog=function(c,d,e){d||(d=["image/*","text/plain"]);this.filePicker.pick(goog.bind(function(d){this.closeEditor();d.url=d.url.replace("https://","http://");d.url=silex.Helper.getAbsolutePath(d.url,silex.Helper.BaseUrl);(new goog.async.Delay(function(){c&&c(d)},10,this)).start()},this),function(c){console.error(c);e&&e(c)});this.openEditor()};
silex.view.FileExplorer.prototype.saveAsDialog=function(c,d,e){d||(d={mimetype:"text/html"});this.filePicker.exportFile("http://google.com/",d,goog.bind(function(d){this.closeEditor();d.url=d.url.replace("https://","http://");d.url=silex.Helper.getAbsolutePath(d.url,silex.Helper.BaseUrl);(new goog.async.Delay(function(){c&&c(d)},10,this)).start()},this),function(c){console.error(c);e&&e(c)});this.openEditor()};
...
c.getAttribute("href").indexOf("fonts")&&c.parentNode.removeChild(c)});var e=this.getNeededFonts();console.log("fonts ",e);c=goog.dom.getElementsByTagNameAndClass("font",null,d);goog.array.forEach(c,function(c){null!==c.getAttribute("face")&&(e[c.getAttribute("face")]=!0)});var f=silex.model.Config.fonts;getFont=function(c){for(var d in f)if(f[d].value===c)return f[d];return null};for(var h in e)if((c=getFont(h))&&void 0!==c.href){var k=goog.dom.createElement("link");k.setAttribute("href",c.href);
k.setAttribute("rel","stylesheet");k.setAttribute("type","text/css");d.appendChild(k);k=goog.dom.createElement("link");k.setAttribute("href",c.href);k.setAttribute("rel","stylesheet");k.setAttribute("type","text/css");document.head.appendChild(k)}};
silex.model.File.prototype.getNeededFonts=function(){var c=this.getStageComponent().getHtml();console.log("getNeededFonts");var d=[];c.replace(/<font.*face="?([^"]*)"/g,function(c,f,h){console.log("found",f);d[f]=!0;return c});console.log("getNeededFonts returns",d);return d};silex.model.File.prototype.getStageComponent=function(){return new silex.model.Component(this.stage.bodyElement)};silex.model.File.prototype.getBodyTag=function(){return this.bodyTag};
silex.model.File.prototype.setBodyTag=function(c){this.bodyTag=c};silex.model.File.prototype.getHeadTag=function(){return this.headTag};silex.model.File.prototype.setHeadTag=function(c){this.headTag=c};silex.model.File.prototype.getUrl=function(){return this.url};silex.model.File.prototype.setUrl=function(c){this.url=c};silex.model.File.prototype.getBlob=function(){return this.blob};silex.model.File.prototype.setBlob=function(c){(this.blob=c)?this.propertiesTool.setBaseUrl(c.url):this.propertiesTool.setBaseUrl(null)};
silex.model.File.prototype.getBodyStyle=function(){return this.getStageComponent().relative2absolute(this.bodyStyle,this.getUrl())};silex.model.File.prototype.setBodyStyle=function(c){this.bodyStyle=this.getStageComponent().absolute2Relative(c,this.getUrl())};
