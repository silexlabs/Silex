...
silex.model.Component.prototype.relative2absolute=function(c,d){console.log("relative2absolute",d);c=c.replace(/src="?([^" ]*)" /g,function(c,f,h){h=c.replace(f,silex.Helper.getAbsolutePath(f,d));console.log("relative2absolute",c,f,h);return h});return c=c.replace(/url\((['"])(.+?)\1\)/g,function(c,f,h){c="url('"+silex.Helper.getAbsolutePath(h,d)+"')";console.log("relative2absolute",h,c);return c})};
...
c.getAttribute("href").indexOf("fonts")&&c.parentNode.removeChild(c)});var e=this.getNeededFonts();console.log("fonts ",e);c=goog.dom.getElementsByTagNameAndClass("font",null,d);goog.array.forEach(c,function(c){null!==c.getAttribute("face")&&(e[c.getAttribute("face")]=!0)});var f=silex.model.Config.fonts,c=function(c){for(var d in f)if(f[d].value===c)return f[d];return null},h;for(h in e){var k=c(h);if(k&&void 0!==k.href){var l=goog.dom.createElement("link");l.setAttribute("href",k.href);l.setAttribute("rel",
"stylesheet");l.setAttribute("type","text/css");d.appendChild(l);l=goog.dom.createElement("link");l.setAttribute("href",k.href);l.setAttribute("rel","stylesheet");l.setAttribute("type","text/css");document.head.appendChild(l)}}};silex.model.File.prototype.getNeededFonts=function(){var c=this.getStageComponent().getHtml();console.log("getNeededFonts");var d=[];c.replace(/<font.*face="?([^"]*)"/g,function(c,f,h){console.log("found",f);d[f]=!0;return c});console.log("getNeededFonts returns",d);return d};
...
silex.model.File.prototype.setUrl=function(c){this.url=c};silex.model.File.prototype.getBlob=function(){return this.blob};silex.model.File.prototype.setBlob=function(c){(this.blob=c)?this.propertiesTool.setBaseUrl(c.url):this.propertiesTool.setBaseUrl(null)};silex.model.File.prototype.getBodyStyle=function(){return this.getStageComponent().relative2absolute(this.bodyStyle,this.getUrl())};
silex.model.File.prototype.setBodyStyle=function(c){this.bodyStyle=this.getStageComponent().absolute2Relative(c,this.getUrl())};
...
e).remove();f=goog.dom.getElementsByClass("editable-style",m);goog.array.forEach(f,function(c){c=new silex.model.Component(c);var d=c.element.getAttribute("data-silex-href");d&&(c.element.setAttribute("href",d),c.element.removeAttribute("data-silex-href"),d=goog.dom.getOuterHtml(c.element),d="<a"+d.substring(4,d.length-6)+"</a>",d=goog.dom.htmlToDocumentFragment(d),goog.dom.insertSiblingBefore(d,c.element),goog.dom.removeNode(c.element),c.element=d)},this);var f=goog.dom.getElementsByClass("editable-style",
m),n=0;goog.array.forEach(f,function(c){c=new silex.model.Component(c);c.addClass("silex-"+c.type);var d="component-"+n++;c.addClass(d);var e=c.getCss(silex.model.Component.CONTEXT_NORMAL);h.push({classNames:["."+d],styles:e});c.hasStyle(silex.model.Component.CONTEXT_HOVER)&&(e=c.getCss(silex.model.Component.CONTEXT_HOVER),h.push({classNames:["."+d+":hover"],styles:e}));c.hasStyle(silex.model.Component.CONTEXT_PRESSED)&&(e=c.getCss(silex.model.Component.CONTEXT_PRESSED),h.push({classNames:["."+d+
":pressed"],styles:e}));c.removeClass("editable-style");c.element.removeAttribute("data-silex-type");c.element.removeAttribute("data-silex-sub-type");c.element.removeAttribute("data-style-"+silex.model.Component.CONTEXT_NORMAL);c.element.removeAttribute("data-style-"+silex.model.Component.CONTEXT_HOVER);c.element.removeAttribute("data-style-"+silex.model.Component.CONTEXT_PRESSED);c.element.removeAttribute("style")},this);f=this.stage.getBodyStyle();f=f.replace(/url\((['"])(.+?)\1\)/g,goog.bind(function(c,
d,e){return this.filterBgImage(l,k,c,d,e)},this));h.push({classNames:["body"],styles:silex.Helper.stringToStyle(f)});var p="";goog.array.forEach(h,function(c){var d="";goog.array.forEach(c.classNames,function(c){""!=d&&(d+=", ");d+=c},this);d+="{\n\t"+silex.Helper.styleToString(c.styles)+"\n}";p+="\n"+d},this);p.replace("; ",";\n\t");e="<html>"+('<head><link href="css/styles.css" rel="stylesheet">'+e.innerHTML+"</head>");e+="<body>"+m.innerHTML+"</body>";e+="</html>";c(e,p,k)}else d&&d({message:"The file must be saved before I can clean it up for you."})};
