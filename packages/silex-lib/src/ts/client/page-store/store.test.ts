...
import { createPages, deletePages, getPages, initializePages, movePage, updatePages } from './index';
...
test('Update a page', () => {
  ...
  expect(getPages()[0].id).toBe(PAGE1.id)
  expect(getPages()[1].id).toBe(PAGE2.id)
  updatePages([{from: getPages()[1], to: getPages()[1]}])
  expect(getPages()).toHaveLength(2)
  expect(() => updatePages([{from: getPages()[1], to: PAGE3}])).toThrow()
})
