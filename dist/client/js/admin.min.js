...
silex.model.Element.prototype.getType=function(c){return c.getAttribute(silex.model.Element.TYPE_ATTR)};silex.model.Element.prototype.setType=function(c,d){console.error("Error: not implemented");throw"Error: not implemented";};silex.model.Element.prototype.getStyle=function(c,d){return c.style[d]};
silex.model.Element.prototype.setStyle=function(c,d,e){console.log(c,d,e,c.style[d],goog.isDef(e));goog.isDef(e)?(c.style[d]=e,console.log(c,d,e,c.style.backgroundAttachment,goog.isDef(e))):c.style[d]=""};silex.model.Element.prototype.setBgImage=function(c,d){this.setStyle(c,"backgroundImage","url("+d+")")};silex.model.Element.prototype.getInnerHtml=function(c){silex.utils.EditablePlugin.setEditable(c,!1);var d=this.getContentNode(c).innerHTML;silex.utils.EditablePlugin.setEditable(c,!0);return d};
silex.model.Element.prototype.setInnerHtml=function(c,d){var e=this.getContentNode(c);silex.utils.EditablePlugin.setEditableHtml(e,d)};silex.model.Element.prototype.getContentNode=function(c){var d=goog.dom.getElementsByClass(silex.model.Element.ELEMENT_CONTENT_CLASS_NAME,c);return d&&1===d.length?d[0]:c};
...
silex.utils.Notification.notifySuccess=function(c){console.info(c);silex.utils.Notification.nativeNotification(c,silex.utils.Notification.SUCCESS_ICON);alertify.success(c)};silex.utils.Notification.notifyError=function(c){console.error(c);silex.utils.Notification.nativeNotification(c,silex.utils.Notification.ERROR_ICON);alertify.error(c)};silex.utils.Notification.notifyInfo=function(c){console.info(c);silex.utils.Notification.nativeNotification(c,silex.utils.Notification.INFO_ICON);alertify.log(c)};silex.utils.PageablePlugin=function(){throw"this is a static class and it canot be instanciated";};silex.utils.PageablePlugin.getBodyElement=function(){return silex.utils.PageablePlugin.bodyElement};silex.utils.PageablePlugin.setBodyElement=function(c){silex.utils.PageablePlugin.bodyElement=c};silex.utils.PageablePlugin.LINK_ATTR="data-silex-href";silex.utils.PageablePlugin.PAGEABLE_ROOT_CLASS="pageable-root-class";silex.utils.PageablePlugin.PAGE_CLASS="silex-page";
silex.utils.PageablePlugin.getParentPage=function(c){for(c=c.parentNode;c&&!goog.dom.classes.has(c,silex.utils.PageablePlugin.PAGE_CLASS);)c=c.parentNode;return c};silex.utils.PageablePlugin.getPageable=function(c){return c&&$(c).hasClass(silex.utils.PageablePlugin.PAGEABLE_ROOT_CLASS)};
silex.utils.PageablePlugin.setPageable=function(c,d){if(d){$(c).addClass(silex.utils.PageablePlugin.PAGEABLE_ROOT_CLASS);var e=silex.utils.PageablePlugin.getPages();$(c).pageable({currentPage:e[0],useDeeplink:!1})}else $(c).pageable("destroy"),$(c).removeClass(silex.utils.PageablePlugin.PAGEABLE_ROOT_CLASS)};
...
"insert."+c,-1),console.error("could not add element of type",c," - ",error)}return d};silex.controller.ControllerBase.prototype.openCssEditor=function(c){this.view.cssEditor.openEditor(this.model.head.getHeadStyle())};
...
(d.message||""));this.tracker.trackAction("controller-events","error",c,-1)},this));this.view.workspace.invalidate();break;case "selectImage":this.editElement();break;case "styleChanged":var f=this.view.stage.getSelection()[0];f&&d?(console.log(arguments),console.log(f.style.backgroundAttachment),this.model.element.setStyle(f,d,e),console.log(f.style.backgroundAttachment),this.view.propertyTool.redraw()):console.error("can not set style ",d," on element ",f);break;case "propertyChanged":console.error("not implemented");
break;case "addToPage":silex.utils.PageablePlugin.addToPage(this.view.stage.getSelection()[0],d);break;case "removeFromPage":silex.utils.PageablePlugin.removeFromPage(this.view.stage.getSelection()[0],d);break;case "addLink":silex.utils.PageablePlugin.setLink(this.view.stage.getSelection()[0],d);break;case "removeLink":silex.utils.PageablePlugin.setLink(this.view.stage.getSelection()[0])}};silex.controller.MenuController=function(c,d){silex.controller.ControllerBase.call(this,c,d);d.menu.onStatus=goog.bind(this.menuCallback,this)};goog.inherits(silex.controller.MenuController,silex.controller.ControllerBase);
...
goog.ui.Component.EventType.ACTION,this.onBgColorButton,!1,this);goog.events.listen(this.transparentBgCheckbox,goog.ui.Component.EventType.CHANGE,this.onTransparentChanged,!1,this);goog.events.listen(c,goog.events.EventType.CLICK,this.onSelectImageButton,!1,this);goog.events.listen(d,goog.events.EventType.CLICK,this.onClearImageButton,!1,this);goog.events.listen(this.attachementComboBox,goog.ui.Component.EventType.CHANGE,function(c){console.log("backgroundAttachment",c.target.getSelectedItem().getId());
this.styleChanged("backgroundAttachment",c.target.getSelectedItem().getId())},!1,this);goog.events.listen(this.vPositionComboBox,goog.ui.Component.EventType.CHANGE,function(c){c=this.hPositionComboBox.getSelectedItem().getId();var d=this.vPositionComboBox.getSelectedItem().getId();this.styleChanged("backgroundPosition",d+" "+c)},!1,this);goog.events.listen(this.hPositionComboBox,goog.ui.Component.EventType.CHANGE,function(c){c=this.hPositionComboBox.getSelectedItem().getId();var d=this.vPositionComboBox.getSelectedItem().getId();
this.styleChanged("backgroundPosition",d+" "+c)},!1,this);goog.events.listen(this.repeatComboBox,goog.ui.Component.EventType.CHANGE,function(c){this.styleChanged("backgroundRepeat",c.target.getSelectedItem().getId())},!1,this)};
...
null!==c.style.backgroundImage&&"none"!==c.style.backgroundImage&&""!==c.style.backgroundImage?this.bgClearBgImage.setEnabled(!0):this.bgClearBgImage.setEnabled(!1);this.bgClearBgImage.setEnabled(!0);console.log(c.style.backgroundAttachment);if(c.style.backgroundAttachment)switch(c.style.backgroundAttachment){case "scroll":this.attachementComboBox.setSelectedIndex(0);break;case "fixed":this.attachementComboBox.setSelectedIndex(1);break;case "local":this.attachementComboBox.setSelectedIndex(2)}else this.attachementComboBox.setSelectedIndex(0);
...
this.element),this.bodyElement,this.headElement);this.propertyPane.onStatus=c;this.pagePane=new silex.view.pane.PagePane(goog.dom.getElementByClass("page-editor",this.element),this.bodyElement,this.headElement);this.pagePane.onStatus=c;this.generalStylePane=new silex.view.pane.GeneralStylePane(goog.dom.getElementByClass("general-editor",this.element),this.bodyElement,this.headElement);this.generalStylePane.onStatus=c};
silex.view.PropertyTool.prototype.redraw=function(){this.borderPane.redraw();this.propertyPane.redraw();this.pagePane.redraw();this.generalStylePane.redraw();this.bgPane.redraw()};silex.view.PropertyTool.prototype.getBaseUrl=function(){return this.baseUrl};silex.view.PropertyTool.prototype.setBaseUrl=function(c){this.baseUrl=c;this.borderPane.setBaseUrl(c);this.propertyPane.setBaseUrl(c);this.pagePane.setBaseUrl(c);this.generalStylePane.setBaseUrl(c);this.bgPane.setBaseUrl(c)};silex.view.PageTool=function(c,d,e){silex.view.ViewBase.call(this,c,d,e);this.initEvents()};goog.inherits(silex.view.PageTool,silex.view.ViewBase);
