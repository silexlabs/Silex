...
silex.controller.ControllerBase.prototype.addElement=function(c){this.tracker.trackAction("controller-events","request","insert."+c,0);try{var d=this.model.element.createElement(c),e=silex.utils.PageablePlugin.getCurrentPageName();silex.utils.PageablePlugin.addToPage(d,e);this.checkElementVisibility(d);this.model.element.setSelected(d,!0);this.view.propertyTool.redraw();this.tracker.trackAction("controller-events","success","insert."+c,1)}catch(f){this.tracker.trackAction("controller-events","error",
"insert."+c,-1),console.error("could not add element of type",c," - ",error)}return d};silex.controller.ControllerBase.prototype.styleChanged=function(c,d){var e=this.view.stage.getSelection()[0];e&&c?(this.model.element.setStyle(e,c,d),this.view.propertyTool.redraw()):console.error("can not set style ",c," on element ",e)};
...
silex.utils.EditablePlugin.getFirstEditableParent=function(c){for(var d=c;d&&d.getAttribute&&!$(d).hasClass(silex.utils.EditablePlugin.EDITABLE_CLASS_NAME);)d=d.parentNode;if(d&&d.getAttribute&&$(d).hasClass(silex.utils.EditablePlugin.EDITABLE_CLASS_NAME))return d;console.warn("this element has no editable parent",c);return c};
silex.utils.EditablePlugin.setEditable=function(c,d,e){console.log("setEditable",arguments);d?($(".editable-style",c).each(function(){"container"===this.getAttribute("data-silex-type")?$(this).editable({isContainer:!0}):$(this).editable()}),"container"===c.getAttribute("data-silex-type")?e?$(c).editable({isContainer:!0,isResizable:!1,isDroppable:!0,isDraggable:!1}):$(c).editable({isContainer:!0}):$(c).editable()):($(".editable-style",c).editable("destroy"),$(c).editable("destroy"),$(c).find(silex.model.Element.SELECTED_CLASS_NAME).removeClass(silex.utils.EditablePlugin.SILEX_SELECTED_CLASS_NAME),
