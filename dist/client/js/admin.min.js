...
silex.utils.Url.absolute2Relative=function(c,d){c=c.replace(/src="?([^" ]*)" /g,function(c,f,h){return c.replace(f,silex.utils.Url.getRelativePath(f,d))});return c=c.replace(/url\((['"])(.+?)\1\)/g,function(c,f,h){return"url('"+silex.utils.Url.getRelativePath(h,d)+"')"})};
silex.utils.Url.relative2absolute=function(c,d){c=c.replace(/src="?([^" ]*)" /g,function(c,f,h){return c.replace(f,silex.utils.Url.getAbsolutePath(f,d))});return c=c.replace(/url\((['"])(.+?)\1\)/g,function(c,f,h){return"url('"+silex.utils.Url.getAbsolutePath(h,d)+"')"})};
...
silex.service.SilexTasks.prototype.publish=function(c,d,e,f,h,k){if(c&&d&&e&&f){var l=new goog.Uri.QueryData;l.add("path",c);l.add("html",d);l.add("css",e);l.add("files",JSON.stringify(f));goog.net.XhrIo.send("/silex/tasks/publish",function(c){c=c.target;if(c.isSuccess()){var d=c.getResponseJson();d.success?h&&h(d):(d=d.code||d.message,console.error(d,c,c.isSuccess(),c.getStatus(),c.headers.toString()),k&&k(d))}else d=c.getLastError(),console.error(c.getLastError(),c.getLastErrorCode(),c.isSuccess(),
c.getStatus(),c.headers),k&&k(d)},"POST",l.toString())}else console.error("Param path, html, css or files missing"),k&&k("Param path, html, css or files missing")};silex.model.File=function(c,d){silex.model.ModelBase.call(this,c,d)};goog.inherits(silex.model.File,silex.model.ModelBase);silex.model.File.CREATION_TEMPLATE="creation-template.html";
...
silex.model.File.prototype.setUrl=function(c){if(c){var d=silex.utils.Url.getBaseUrl();c=silex.utils.Url.getAbsolutePath(c,d)}this.url=c};silex.model.File.prototype.publish=function(c,d,e){this.cleanup(goog.bind(function(f,h,k){silex.service.SilexTasks.getInstance().publish(c,f,h,k,d,e)},this),goog.bind(function(c){console.error("publish cleanup error",c);e&&e(c)},this))};
...
function(c){""!=d&&(d+=", ");d+=c},this);d+="{\n\t"+silex.utils.Style.styleToString(c.styles)+"\n}";p+="\n"+d},this);p.replace("; ",";\n\t");f="<html>"+('<head><link href="css/styles.css" rel="stylesheet">'+f.innerHTML+"</head>");f+="<body>"+e.innerHTML+"</body>";f+="</html>";c(f,p,l)}else d&&d({message:"The file must be saved before I can clean it up for you."})};
