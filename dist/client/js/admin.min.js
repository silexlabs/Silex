...
"text/plain"],goog.bind(function(c){silex.utils.Notification.notifyError("Error: I did not manage to load the image. <br /><br />"+(c.message||""))},this)),this.view.workspace.invalidate()}};silex.controller.ControllerBase.prototype.openPage=function(c){silex.utils.PageablePlugin.setCurrentPage(c);this.view.pageTool.redraw()};
...
silex.controller.ControllerBase.prototype.newFile=function(c,d){this.tracker.trackAction("controller-events","request","file.new",0);this.model.file.newFile(goog.bind(function(d){this.model.file.setHtml(d);this.fileOperationSuccess(null,!0);silex.utils.RetroCompat.process(this.model.body.bodyElement,this.model.head.headElement);this.tracker.trackAction("controller-events","success","file.new",1);c&&c()},this),function(c){this.tracker.trackAction("controller-events","error","file.new",-1);d&&d(c)})};
silex.controller.ControllerBase.prototype.openFile=function(c,d){this.tracker.trackAction("controller-events","request","file.open",0);this.view.fileExplorer.openDialog(goog.bind(function(e){this.model.file.open(e,goog.bind(function(d){d=silex.utils.Url.relative2absolute(d,silex.utils.Url.getBaseUrl(e));this.model.file.setHtml(d);this.fileOperationSuccess(this.model.head.getTitle()+" opened.",!0);this.tracker.trackAction("controller-events","success","file.open",1);c&&c()},this),goog.bind(function(c){silex.utils.Notification.notifyError("Error: I did not manage to open this file. <br /><br />"+
(c.message||""));this.tracker.trackAction("controller-events","error","file.open",-1);d&&d(c)},this))},this),["text/html","text/plain"],function(c){this.tracker.trackAction("controller-events","error","file.open",-1);d&&d(c)})};
...
silex.controller.SettingsDialogController.prototype.settingsDialogCallback=function(c,d){switch(c){case "browsePublishPath":this.view.fileExplorer.openDialog(goog.bind(function(d){d=d.substring(d.indexOf("/api/v1.0/"),d.lastIndexOf("/"));d=d.replace("/exec/get","/exec/put");this.model.head.setPublicationPath(d);this.view.settingsDialog.redraw();this.tracker.trackAction("controller-events","success",c,1)},this),null,goog.bind(function(d){silex.utils.Notification.notifyError("Error: I could not select the publish path. <br /><br />"+
...
silex.controller.PropertyToolController.prototype.propertyToolCallback=function(c,d,e){switch(c){case "editHTML":this.editElement();break;case "editText":this.editElement();break;case "selectBgImage":this.view.fileExplorer.openDialog(goog.bind(function(d){var e=this.view.stage.getSelection()[0];this.model.element.setBgImage(e,d);this.view.propertyTool.redraw();this.tracker.trackAction("controller-events","success",c,1)},this),["image/*","text/plain"],goog.bind(function(d){silex.utils.Notification.notifyError("Error: I could not load the image. <br /><br />"+
...
silex.view.FileExplorer.prototype.openDialog=function(c,d,e){var f;d&&(0===d[0].indexOf("image")?f=["jpg","jpeg","gif","png"]:0===d[0].indexOf("text/html")&&(f=["html","htm"]));var h=function(c){console.error(c);e&&e(c)},k=function(d){(new goog.async.Delay(function(){c&&c(d)},10,this)).start()};this.filePicker.pick(goog.bind(function(c){this.closeEditor();c.url=c.url.replace("https://","http://");if(f&&!1===silex.utils.Url.checkFileExt(c.url,f)){var d=c.url.substring(c.url.lastIndexOf("/")+1);silex.utils.Notification.confirm("The file name "+
d+" does not looks good to me, are you sure you want to select this file?",function(d){d?k(c.url):h({message:"Wrong file type."})})}else k(c.url)},this),h);this.openEditor()};
silex.view.FileExplorer.prototype.saveAsDialog=function(c,d,e){var f;d&&(0===d[0].indexOf("image")?f=["jpg","jpeg","gif","png"]:0===d[0].indexOf("text/html")&&(f=["html","htm"]));var h=function(c){console.error(c);e&&e(c)},k=function(d){(new goog.async.Delay(function(){c&&c(d.url)},10,this)).start()};this.filePicker.exportFile("http://google.com/",d,goog.bind(function(c){this.closeEditor();c.url=c.url.replace("https://","http://");if(f&&!1===silex.utils.Url.checkFileExt(c.url,f)){var d=c.url.substring(c.url.lastIndexOf("/")+
1);silex.utils.Notification.confirm("The file name "+d+" does not looks good to me, are you sure you want to select this file?",function(d){d?k(c):h({message:"Wrong file type."})})}else k(c)},this),h);this.openEditor()};silex.view.FileExplorer.prototype.openEditor=function(){var c=goog.dom.getElementByClass("dialogs-background");goog.style.setStyle(c,"display","inherit");goog.style.setStyle(this.element,"display","");goog.events.listen(c,goog.events.EventType.CLICK,this.closeEditor,!0,this)};
