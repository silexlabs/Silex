...
if (!globalThis.__dirname) {
    globalThis.__dirname = (0, path_1.dirname)(process.cwd() + '/server/connectors/FsStorage.ts');
...
class FsStorage {
    ...
    async initFs() {
        const defaultSite = (0, path_1.join)(this.options.path, constants_1.DEFAULT_WEBSITE_ID);
        const stat = await promises_1.default.stat(defaultSite).catch(() => null);
        if (!stat) {
            await promises_1.default.mkdir((0, path_1.join)(defaultSite, this.options.assetsFolder), { recursive: true });
            await this.setWebsiteMeta({}, constants_1.DEFAULT_WEBSITE_ID, { name: 'Default website', connectorUserSettings: {} });
            await this.updateWebsite({}, constants_1.DEFAULT_WEBSITE_ID, types_1.EMPTY_WEBSITE);
            console.info(`> [FsStorage] Created ${constants_1.DEFAULT_WEBSITE_ID} in ${this.options.path}`);
        }
    }
    ...
    async setWebsiteMeta(session, id, data) {
        ...
        const content = (0, websiteDataSerialization_1.stringify)(data);
    ...
    async createWebsite(session, meta) {
        ...
        await this.updateWebsite(session, id, types_1.EMPTY_WEBSITE);
        return id;
    }
    async readWebsite(session, websiteId) {
        ...
        const pageLoader = async (pagePath) => {
            const fullPath = (0, path_1.join)(this.options.path, id, pagePath);
            const pageContent = await promises_1.default.readFile(fullPath);
    ...
    async updateWebsite(session, websiteId, data) {
        const id = (0, validation_1.requiredParam)(websiteId, 'website id');
        const websitePath = (0, path_1.join)(this.options.path, id);
        ...
        await promises_1.default.mkdir(websitePath, { recursive: true });
        const hasPageFiles = filesToWrite.some(f => f.path.startsWith(pagesFolder));
        if (hasPageFiles) {
            await promises_1.default.mkdir((0, path_1.join)(websitePath, pagesFolder), { recursive: true });
        }
        const pagesPath = (0, path_1.join)(websitePath, pagesFolder);
        try {
            const existingPageFiles = await promises_1.default.readdir(pagesPath);
            ...
            for (const existingFile of existingPageFiles) {
                if (existingFile.endsWith('.json') && !newPageFiles.has(existingFile)) {
                    await promises_1.default.unlink((0, path_1.join)(pagesPath, existingFile));
                }
            }
        }
        catch (error) {
        ...
        for (const file of filesToWrite) {
            const filePath = (0, path_1.join)(websitePath, file.path);
