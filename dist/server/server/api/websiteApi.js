...
function default_1(config, opts = {}) {
    ...
    router.post(constants_1.API_WEBSITE_FORK, async (req, res) => {
        try {
            const query = req.query;
            const body = req.body;
            const gitlabUrl = (0, validation_1.requiredParam)(body.gitlabUrl, 'GitLab project path ("username/repo")');
            const websiteId = await forkWebsite(req['session'], gitlabUrl, query.connectorId);
            res.status(200).json({ websiteId, message: 'Website forked successfully' });
        }
        catch (e) {
    ...
    async function forkWebsite(session, gitlabUrl, connectorId) {
        const storageConnector = await getStorageConnector(session, connectorId);
        if (typeof storageConnector.forkWebsite !== 'function') {
            throw new types_1.ApiError('This storage connector does not support forking external projects', 400);
        }
        return storageConnector.forkWebsite(session, gitlabUrl);
    }
