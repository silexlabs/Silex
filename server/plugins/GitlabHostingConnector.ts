...
import GitlabConnector, { GitlabOptions, GitlabSession, computeGitBlobSha } from './GitlabConnector.js'
...
import { BUILD_JSON_PATH, BUILD_SH_PATH, DEFAULT_BUILD_STEPS, generateBuildSh, parseBuildSteps } from '~/server/utils/buildPipeline.js'
...
const PATH_YML = '.gitlab-ci.yml'
const CONTENT_YML = dedent`
  ${SILEX_OVERWRITE_NOTICE}
  # Generated by Silex: customize the build in build.json, not here
  image: node:20
  pages:
    stage: deploy
...
export default class GitlabHostingConnector extends GitlabConnector implements HostingConnector {
  ...
  /**
   * Generate the pipeline files: build.json belongs to the user (created once,
   * never rewritten), build.sh and the CI file belong to Silex
   *
   * Decisions are made from the SHAs of the repository listing, so an
   * up-to-date site costs no extra request
   */
  async setupPipeline(session: GitlabSession, websiteId: WebsiteId, existingFiles: Map<string, string>, job: PublicationJobData): Promise<void> {
    const log = (message: string) => {
      job.message = message
      job.logs[0].push(message)
    }

    const defaultJson = JSON.stringify(DEFAULT_BUILD_STEPS, null, 2) + '\n'
    let steps = DEFAULT_BUILD_STEPS
    const jsonSha = existingFiles.get(BUILD_JSON_PATH)
    if (!jsonSha) {
      log(`Creating ${BUILD_JSON_PATH}...`)
      await this.createFile(session, websiteId, BUILD_JSON_PATH, defaultJson)
    } else if (jsonSha !== computeGitBlobSha(defaultJson, false)) {
      steps = parseBuildSteps((await this.readFile(session, websiteId, BUILD_JSON_PATH)).toString())
    }

    const buildSh = generateBuildSh(steps)
    const shSha = existingFiles.get(BUILD_SH_PATH)
    if (!shSha) {
      log(`Creating ${BUILD_SH_PATH}...`)
      await this.createFile(session, websiteId, BUILD_SH_PATH, buildSh)
    } else if (shSha !== computeGitBlobSha(buildSh, false)) {
      log(`Updating ${BUILD_SH_PATH}...`)
      await this.updateFile(session, websiteId, BUILD_SH_PATH, buildSh)
    }

    const ymlSha = existingFiles.get(PATH_YML)
    if (!ymlSha) {
      log(`Creating ${PATH_YML}...`)
      await this.createFile(session, websiteId, PATH_YML, CONTENT_YML)
    } else if (ymlSha !== computeGitBlobSha(CONTENT_YML, false)) {
      // The marker means Silex owns the file; without it the user customized
      // it before build.json existed, and it keeps working untouched
      const existingYml = await this.readFile(session, websiteId, PATH_YML)
      if (existingYml.toString().startsWith(SILEX_OVERWRITE_NOTICE)) {
        log(`Updating ${PATH_YML}...`)
        await this.updateFile(session, websiteId, PATH_YML, CONTENT_YML)
      }
    }
  }
  ...
  async publish(session: GitlabSession, websiteId: WebsiteId, files: ConnectorFile[], {startJob, jobSuccess, jobError}: JobManager): Promise<JobData> {
    ...
    job.errors = [[]]
    // One repository listing shared by the pipeline decisions and writeAssets
    let existingFiles: Map<string, string>
    try {
      existingFiles = await this.ls({ session, websiteId, recursive: true })
      await this.setupPipeline(session, websiteId, existingFiles, job)
    } catch (e) {
      jobError(job.jobId, e.message)
      return job
    }
    const assetsFiles = new Map([...existingFiles].filter(([path]) => path.startsWith(`${this.options.assetsFolder}/`)))
