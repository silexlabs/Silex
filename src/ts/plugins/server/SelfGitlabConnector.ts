...
/**
 * Self Hosted Gitlab connector
 * @fileoverview Self Hosted Gitlab connector for Silex, connect to the user's Self Hosted Gitlab account to store websites
 * @see https://docs.gitlab.com/ee/api/oauth2.html with <gitlab.example.com>
 */
...
<svg
  ...
  <g
     id="LOGO"
     transform="matrix(5.2068817,0,0,5.2068817,-489.30757,-507.76085)">
    <path
       class="cls-1"
  ...
  <text
     xml:space="preserve"
     style="font-style:normal;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:96px;line-height:0;font-family:Laksaman;-inkscape-font-specification:'Laksaman Bold';white-space:pre;opacity:0.99;fill:#2b0000;fill-opacity:1;stroke-width:37.795;stroke-dasharray:none"
     id="text1-1"
     x="406.83264"
     y="411.44205"><tspan
       id="tspan1-3"
       style="fill:#2b0000;fill-opacity:1;stroke-width:37.795;stroke-dasharray:none">Self</tspan></text>
  <text
     xml:space="preserve"
     style="font-style:normal;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:96px;line-height:0;font-family:Laksaman;-inkscape-font-specification:'Laksaman Bold';white-space:pre;opacity:0.99;fill:#2b0000;fill-opacity:1;stroke-width:37.795;stroke-dasharray:none"
     id="text1"
     ...
     y="803.63361"><tspan
       id="tspan1"
       style="fill:#2b0000;fill-opacity:1;stroke-width:37.795;stroke-dasharray:none">hosted</tspan></text>
  <ellipse
     style="opacity:0.99;fill:#2b0000;fill-opacity:1;stroke-width:37.5323;stroke-dasharray:none"
     id="path1"
     cx="-578.31201"
     cy="497.74338"
     rx="141.10237"
...
export default class GitlabConnector implements StorageConnector {
  connectorId = 'selfGitlab'
  connectorType = ConnectorType.STORAGE
  displayName = 'Self-GitLab'
  icon = ICON
  disableLogout = false
  color = '#2B1B63'
  ...
  constructor(private config: ServerConfig, opts: Partial<GitlabOptions>) {
    ...
    if(!this.options.clientId) throw new Error('Missing Self-Gitlab client ID')
    if(!this.options.clientSecret) throw new Error('Missing Self-Gitlab client secret')
    if(!this.options.domain) throw new Error('Missing Self-Gitlab domain')
  }
  ...
  async readFile(session: GitlabSession, websiteId: string, fileName: string): Promise<Buffer> {
    ...
    if(!response.ok) throw new ApiError(`Error reading file "${fileName}" from Self-Gitlab: ${json?.message ?? json?.error ?? response.statusText}`, response.status)
  ...
  private async callApi(session: GitlabSession, path: string, method: 'POST' | 'GET' | 'PUT' | 'DELETE' = 'GET', body: GitlabWriteFile | GitlabGetToken | GitlabWebsiteName | GitlabCreateBranch | null = null, params: any = {}): Promise<any> {
    ...
    if(method === 'GET' && body) {
      console.error('Self-Gitlab API error (4) - GET request with body', {url, method, body, params})
    }
    ...
    if(!response.ok) {
      if (text.includes('A file with this name doesn\'t exist')) {
        throw new ApiError('Self-Gitlab API error (5): Not Found', 404)
        ...
        if (response.ok) {
          ...
          console.error('Self-Gitlab API error (2) - could not refresh token', response.status, response.statusText, {message}, 'refresh_token:', token.refresh_token)
          // Workaround for when the token is invalid
          // It happens often which is not normal (refresh token should last 6 months)
          this.logout(session)
          // Notify the user
          throw new ApiError(`Self-Gitlab API error (2): ${message}`, response.status)
        }
      } else {
        ...
        throw new ApiError(`Self)Gitlab API error (1): ${message}`, response.status)
      }
    }
    try {
      ...
      if(!response.ok) {
        ...
        console.error('Self-Gitlab API error (3) - could not parse response', response.status, response.statusText, {url, method, body, params, text})
        return text
      }
    }
    return json
